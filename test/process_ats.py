...
engine = create_engine("mysql+pymysql://vaccine:vaccine@localhost/vaccine-ats", pool_pre_ping=True)
conn = engine.connect()
...
s = select([content.c.id,content.c.link,content.c.raw,threads.c.id,threads.c.link]).where(and_(content.c.thread == threads.c.id,threads.c.domain == "abovetopsecret.com"))
...
with open("ats_links.csv","w") as f:
    writer = csv.writer(f)
